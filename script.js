...
function CALCULAR() {
    ...
    const calculateRow = (row) => {
        ...
        switch(acto) {
            ...
            case "TRÁMITE IGAC":
                //Igac=valorActo.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 });
                Igac=valorActo;
           ...
           // row.children[6].textContent = valorActo.toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 });
                //row.children[6].textContent=toLocaleString('es-CO', { style: 'currency', currency: 'COP', minimumFractionDigits: 0, maximumFractionDigits: 0 });
                Saber=valorActo;
            ...
            row.setAttribute('data-total', Math.round(tributaria + notaryFee));
               ...
               // Actualiza las celdas de la fila
               if (acto !== "TRÁMITE IGAC" && acto !== "ESCRITURA PARA SABER") {
...
/* function calcularTotales() {
    ...
    rows.forEach(row => {
        ...
        console.log(`Fila: ${acto}, Total: ${total}, Igac: ${Igac}, Saber: ${Saber}, ValorActo: ${valorActo}`);
        ...
        if (acto !== "TRÁMITE IGAC" && acto !== "ESCRITURA PARA SABER" && valorActo > 0) {
            contHonorarios++;
    ...
    const totalConsignar = totalSum + honorarios + retiros;

    // Depuración: Imprime los totales para verificar
    console.log(`TotalSum: ${totalSum}, Honorarios: ${honorarios}, Retiros: ${retiros}, Total a consignar: ${totalConsignar}`);
